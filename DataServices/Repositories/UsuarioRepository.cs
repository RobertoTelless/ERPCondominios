...
namespace DataServices.Repositories
{
    public class UsuarioRepository : RepositoryBase<USUARIO>, IUsuarioRepository
    {
        ...
        public USUARIO GetComprador(Int32 idAss)
        {
            ...
            query = query.Include(p => p.CONDOMINIO);
            query = query.Include(p => p.PERFIL);
            return query.FirstOrDefault();
        }
        ...
        public USUARIO GetAprovador(Int32 idAss)
        {
            ...
            query = query.Include(p => p.COND_CD_ID);
            query = query.Include(p => p.PERFIL);
            return query.FirstOrDefault();
        }
        ...
        public List<USUARIO> ExecuteFilter(Int32? causId, Int32? cargoId, Int32? unidId, String nome, String login, String email, Int32 idAss)
        {
            ...
            if (unidId != null)
            {
                query = query.Where(p => p.UNID_CD_ID == unidId);
            }
            if (query != null)
            {
                query = query.Where(p => p.COND_CD_ID == idAss);
